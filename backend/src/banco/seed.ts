...
async function seed(): Promise<void> {
  ...
  try {
    ...
    for (const artigo of artigos) for (const t of prepararListaTags(artigo)) tagsUnicas.add(t);
    ...
    if (resultadoTags.length > 0) {
      ...
      const idxId = colunas.indexOf('id');
      const idxNome = colunas.indexOf('nome');
      for (const linha of linhas) mapaTagParaId.set(String(linha[idxNome]), Number(linha[idxId]));
    }
