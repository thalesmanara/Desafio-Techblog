...
import { criarArtigo } from '../servicos/artigos';

export function PaginaNovoArtigo() {
  const navigate = useNavigate();
  const [titulo, setTitulo] = useState('');
  const [conteudo, setConteudo] = useState('');
  const [tags, setTags] = useState('react, ui');
  const [tagPrincipal, setTagPrincipal] = useState('react');
  ...
  async function salvar() {
    ...
    try {
      ...
      const res = await criarArtigo({ titulo, conteudo, imagemUrl: null, tags: listaTags, tagPrincipal: tagPrincipal.trim() || null });
      navigate(`/artigos/${res.id}`);
    } catch (e: any) {
      setErro(e?.message ?? 'Não foi possível criar o artigo.');
    }
  }
